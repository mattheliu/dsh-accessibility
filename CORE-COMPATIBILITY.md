...
| Partial alpha.1 accessibility candidate | PR [omdsh-dev/deepseek-harness#1](https://github.com/omdsh-dev/deepseek-harness/pull/1), commit `6987ae3e70b1d39ec05050b1e43a436f0e74bac8` | automated review passed after one isolated Windows native-test rerun; incomplete core-port scope |
...
The rc.2 candidate changed 191 paths across the pre-alpha client. Alpha.1 reorganized the Web client and test projections, so path equality is not a compatibility result. The partial alpha.1 candidate changes 202 paths relative to its fork base, including 117 test or snapshot paths. Each behavior still needs an explicit disposition.
...
| Plugin list semantics, JSON trees, and subagent lineage trees | **Pending re-audit** | No alpha.1 disposition is recorded. Hierarchy speech, roving/virtual-cursor operation, state changes, and task effectiveness require review. |
...
The exact merge candidate tested by GitHub Actions is represented by PR #1 at `6987ae3e70b1d39ec05050b1e43a436f0e74bac8`. [Actions run 33291309798, attempt 2](https://github.com/omdsh-dev/deepseek-harness/actions/runs/33291309798) completed successfully across the required accessibility browser matrix, Node 22/24/26 lanes, Linux and Windows exhaustive coverage, Windows build/native/observational and Wine lanes, Python release-shaped matrix, package assembly, snapshots, and aggregate status. The final accessibility browser job is [99205674953](https://github.com/omdsh-dev/deepseek-harness/actions/runs/33291309798/job/99205674953), the snapshot/artifact job is [99205680775](https://github.com/omdsh-dev/deepseek-harness/actions/runs/33291309798/job/99205680775), Linux coverage is [99205666353](https://github.com/omdsh-dev/deepseek-harness/actions/runs/33291309798/job/99205666353), Windows coverage is [99205678903](https://github.com/omdsh-dev/deepseek-harness/actions/runs/33291309798/job/99205678903), Windows native tests are [99205665754](https://github.com/omdsh-dev/deepseek-harness/actions/runs/33291309798/job/99205665754), and the aggregate job is [99205854888](https://github.com/omdsh-dev/deepseek-harness/actions/runs/33291309798/job/99205854888).

The accessibility browser job passed 8 of 8 Chromium scenarios, 7 of 8 Firefox scenarios with the forced-colors case skipped, and 7 of 8 WebKit scenarios with the same capability-bound skip. The snapshot job passed all 91 Web test files, with 307 tests passed and 13 conditionally skipped across 320 tests; the cold-history file-action scenario passed 11 of 12 tests with the record-only case skipped. Linux coverage passed 990 of 997 files and 16,020 of 16,061 tests. Windows coverage passed 957 of 960 files and 15,348 of 15,373 tests. Both platforms reported 100% statements, branches, functions, and lines.

The current run history is retained rather than rewritten as an uninterrupted pass. The first attempt's only failing job was [Windows native tests 99203312942](https://github.com/omdsh-dev/deepseek-harness/actions/runs/33291309798/job/99203312942): one Vitest fork worker exited unexpectedly before the fifth file completed, while the four completed files reported 33 passing tests and one skipped test with no failed assertion. All other jobs, including the complete snapshot, cross-browser accessibility, Linux coverage, and Windows coverage gates, passed. One failed-job rerun on a fresh runner passed all 5 files with 69 tests passed and one skipped, then the aggregate gate passed. No assertion was waived, and the rerun history remains part of the evidence boundary.

The immediately preceding checkpoint is also retained. At `4bbbc5bb115d72e80c538563af4d782bc104e0c6`, [run 33290741103](https://github.com/omdsh-dev/deepseek-harness/actions/runs/33290741103) passed the three-engine accessibility matrix but failed the snapshot job because a role locator tried to discover a file button while its owning Turn was still hidden. That caused two direct timeouts and two downstream shared-page golden mismatches. Commit `6987ae3e70b1d39ec05050b1e43a436f0e74bac8` now locates the DOM row, opens its owning Turn, and only then performs the role query; the final 91-file snapshot pass above changed no golden and waived no assertion.
