...
| alpha.1 部分无障碍候选 | PR [omdsh-dev/deepseek-harness#1](https://github.com/omdsh-dev/deepseek-harness/pull/1)，commit `cb804349a3a825c9ddb848429ee04895398d1490` | 使用精确邮箱确认同步及 suite 级 Windows worker 生命周期进程边界后，最终自动化评审通过；核心移植范围不完整 |
...
rc.2 候选在 alpha 之前的客户端中修改了 191 个路径。alpha.1 重组了 Web 客户端和测试 projection，因此文件路径相同不等于兼容。alpha.1 部分候选相对 fork 基线共修改 265 个路径，其中包括 143 个测试或快照路径。每项行为仍需明确处置。
...
GitHub Actions 测试的精确候选为 PR #1 的 `cb804349a3a825c9ddb848429ee04895398d1490`。[Actions run 33307746119](https://github.com/omdsh-dev/deepseek-harness/actions/runs/33307746119) 无需重跑 job 即通过全部 19 个 job，覆盖必需的无障碍浏览器矩阵、Node 22／24／26、Linux 与 Windows 穷尽覆盖率、Windows 构建／原生／观察性及 Wine 通道、Python 发布形态矩阵、包组装、快照和汇总状态。无障碍浏览器 job 为 [99247190071](https://github.com/omdsh-dev/deepseek-harness/actions/runs/33307746119/job/99247190071)，快照／产物 job 为 [99247190148](https://github.com/omdsh-dev/deepseek-harness/actions/runs/33307746119/job/99247190148)，Linux 覆盖率为 [99247190154](https://github.com/omdsh-dev/deepseek-harness/actions/runs/33307746119/job/99247190154)，Windows 覆盖率为 [99247190107](https://github.com/omdsh-dev/deepseek-harness/actions/runs/33307746119/job/99247190107)，Windows 原生测试为 [99247190184](https://github.com/omdsh-dev/deepseek-harness/actions/runs/33307746119/job/99247190184)，汇总 job 为 [99248586661](https://github.com/omdsh-dev/deepseek-harness/actions/runs/33307746119/job/99248586661)。

无障碍浏览器 job 中，Chromium 8 个场景全部通过；Firefox 通过 8 个场景中的 7 个，按能力边界跳过强制颜色场景；WebKit 同样通过 7 个并跳过 1 个。快照 job 通过 91 个 Web 测试文件中的 90 个，并跳过仅适用于 PowerShell 的 1 个文件；320 个测试中 305 个通过、15 个按条件跳过。Linux 覆盖率通过 998 个文件中的 991 个、16,071 个测试中的 16,030 个，41 个测试按条件跳过；Windows 覆盖率通过 961 个文件中的 958 个、15,383 个测试中的 15,358 个，25 个测试按条件跳过。两个平台的语句、分支、函数和行覆盖率均为 100%。Windows 原生测试刻意把 worker 生命周期文件的 4 个具名 suite 分配到 4 个全新的单文件、单 worker 进程中，分别通过 22、21、7 和 4 个测试，覆盖全部 54 个唯一测试。第 5 个进程运行其余 4 个原生文件，通过 15 个测试并跳过 1 个平台限定测试；各进程不共享 fork 池。
