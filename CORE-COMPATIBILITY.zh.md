...
| alpha.1 部分无障碍候选 | PR [omdsh-dev/deepseek-harness#1](https://github.com/omdsh-dev/deepseek-harness/pull/1)，commit `c45dd9d448f3eb7774afeaec1e20061990381775` | 无需重跑即通过自动化评审；核心移植范围不完整 |
...
rc.2 候选在 alpha 之前的客户端中修改了 191 个路径。alpha.1 重组了 Web 客户端和测试 projection，因此文件路径相同不等于兼容。alpha.1 部分候选相对 fork 基线共修改 211 个路径，其中包括 122 个测试或快照路径。每项行为仍需明确处置。
...
GitHub Actions 测试的精确候选为 PR #1 的 `c45dd9d448f3eb7774afeaec1e20061990381775`。[Actions run 33296286172](https://github.com/omdsh-dev/deepseek-harness/actions/runs/33296286172) 无需重跑即最终成功，覆盖必需的无障碍浏览器矩阵、Node 22／24／26、Linux 与 Windows 穷尽覆盖率、Windows 构建／原生／观察性及 Wine 通道、Python 发布形态矩阵、包组装、快照和汇总状态。无障碍浏览器 job 为 [99216395809](https://github.com/omdsh-dev/deepseek-harness/actions/runs/33296286172/job/99216395809)，快照／产物 job 为 [99216395761](https://github.com/omdsh-dev/deepseek-harness/actions/runs/33296286172/job/99216395761)，Linux 覆盖率为 [99216395799](https://github.com/omdsh-dev/deepseek-harness/actions/runs/33296286172/job/99216395799)，Windows 覆盖率为 [99216395819](https://github.com/omdsh-dev/deepseek-harness/actions/runs/33296286172/job/99216395819)，Windows 原生测试为 [99216395796](https://github.com/omdsh-dev/deepseek-harness/actions/runs/33296286172/job/99216395796)，汇总 job 为 [99217676986](https://github.com/omdsh-dev/deepseek-harness/actions/runs/33296286172/job/99217676986)。

无障碍浏览器 job 中，Chromium 8 个场景全部通过；Firefox 通过 8 个场景中的 7 个，按能力边界跳过强制颜色场景；WebKit 同样通过 7 个并跳过 1 个。快照 job 的 91 个 Web 测试文件全部通过，320 个测试中 307 个通过、13 个按条件跳过；其中冷历史文件操作场景通过 12 个测试中的 11 个，跳过仅用于录制的 1 个。Linux 覆盖率通过 997 个文件中的 990 个、16,064 个测试中的 16,023 个；Windows 覆盖率通过 960 个文件中的 957 个、15,376 个测试中的 15,351 个。两个平台的语句、分支、函数和行覆盖率均为 100%。Windows 原生测试 5 个文件全部通过，69 个测试通过，1 个平台限定测试跳过。
