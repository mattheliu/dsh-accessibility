...
| alpha.1 部分无障碍候选 | PR [omdsh-dev/deepseek-harness#1](https://github.com/omdsh-dev/deepseek-harness/pull/1)，commit `9dfe8b961bf65bb2502b90c66e331e7b81fe1791` | 对 Windows worker 生命周期测试做确定性隔离后，最终自动化评审通过；核心移植范围不完整 |
...
rc.2 候选在 alpha 之前的客户端中修改了 191 个路径。alpha.1 重组了 Web 客户端和测试 projection，因此文件路径相同不等于兼容。alpha.1 部分候选相对 fork 基线共修改 259 个路径，其中包括 141 个测试或快照路径。每项行为仍需明确处置。
...
| 目录浏览器与嵌套新建文件夹弹窗 | 通过共享 `Modal` **已重建并自动验证** | 父级浏览器具有名称，初始输入会获得焦点；打开新建文件夹子弹窗时，父级保持 inert，子级保持可操作。父级控件继续禁用，Escape 只关闭子级，焦点先返回子级触发器，随后父级恢复。精确朗读、虚拟光标／盲文操作、错误处理、路径安全理解和残障开发者任务证据仍待补。 |
| Workspace 重命名／删除、目录错误弹窗及其他 Modal 使用方 | **待重新审计** | 每个剩余使用方都必须证明命名、初始焦点、关闭策略、破坏性操作防护、错误播报、焦点约束、inert 生命周期和焦点恢复，并避免重复管理焦点。 |
...
GitHub Actions 测试的精确候选为 PR #1 的 `9dfe8b961bf65bb2502b90c66e331e7b81fe1791`。[Actions run 33304086558](https://github.com/omdsh-dev/deepseek-harness/actions/runs/33304086558) 无需重跑 job 即通过全部 19 个 job，覆盖必需的无障碍浏览器矩阵、Node 22／24／26、Linux 与 Windows 穷尽覆盖率、Windows 构建／原生／观察性及 Wine 通道、Python 发布形态矩阵、包组装、快照和汇总状态。无障碍浏览器 job 为 [99237469194](https://github.com/omdsh-dev/deepseek-harness/actions/runs/33304086558/job/99237469194)，快照／产物 job 为 [99237469214](https://github.com/omdsh-dev/deepseek-harness/actions/runs/33304086558/job/99237469214)，Linux 覆盖率为 [99237469275](https://github.com/omdsh-dev/deepseek-harness/actions/runs/33304086558/job/99237469275)，Windows 覆盖率为 [99237469260](https://github.com/omdsh-dev/deepseek-harness/actions/runs/33304086558/job/99237469260)，Windows 原生测试为 [99237469251](https://github.com/omdsh-dev/deepseek-harness/actions/runs/33304086558/job/99237469251)，汇总 job 为 [99238856435](https://github.com/omdsh-dev/deepseek-harness/actions/runs/33304086558/job/99238856435)。

无障碍浏览器 job 中，Chromium 8 个场景全部通过；Firefox 通过 8 个场景中的 7 个，按能力边界跳过强制颜色场景；WebKit 同样通过 7 个并跳过 1 个。快照 job 的 91 个 Web 测试文件全部通过，320 个测试中 307 个通过、13 个按条件跳过；其中冷历史文件操作场景通过 12 个测试中的 11 个，跳过仅用于录制的 1 个。Linux 覆盖率通过 998 个文件中的 991 个、16,071 个测试中的 16,030 个，41 个测试按条件跳过；Windows 覆盖率通过 961 个文件中的 958 个、15,383 个测试中的 15,358 个，25 个测试按条件跳过。两个平台的语句、分支、函数和行覆盖率均为 100%。Windows 原生测试刻意把 worker 生命周期文件单独运行（54 个测试全部通过），再运行其余 4 个文件（15 个测试通过、1 个平台限定测试跳过），在不共享 fork 池的情况下保留 5 个文件、69 个通过、1 个跳过的相同总量。
