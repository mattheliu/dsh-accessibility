...
6. Verify that the English and Chinese issue-form URLs open the intended default-branch forms, the private withdrawal route works, and every public link is readable without organization membership.
...
8. Re-run `pnpm test`, `pnpm run typecheck`, `pnpm run evidence:validate`, and `pnpm run evidence:coverage`. Zero human records is the correct starting state.
