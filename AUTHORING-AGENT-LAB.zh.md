...
- 两次模型可见审计结果都附带完整的 `dsh-a11y-author-review-plan/0.1.0-draft`，十一项全部保留为 `unresolved`，且不产生符合性声明，也不让自动化代替直接真人证据；
